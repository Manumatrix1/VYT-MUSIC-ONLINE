...
/**
 * Procesar pago confirmado de VYT-MONEY (webhook de MercadoPago)
 */
exports.processVYTMoneyPayment = onRequest(async (req, res) => {
  try {
    const { payment_id, status, external_reference } = req.body;

    if (status === 'approved' && payment_id && external_reference) {
      const referenceData = JSON.parse(external_reference);
      
      if (referenceData.type === 'vyt_money_purchase') {
        const { user_id, cantidad_vyt_money } = referenceData;
        ...
        if (paymentData.status === 'approved') {
          ...
          console.log(`VYT-MONEY purchase confirmed for user ${user_id}: ${cantidad_vyt_money} VYT-MONEY`);
        }
      }
    }
